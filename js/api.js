...
export const arr=(x,keys=["posts","data","items","citizens","listings"])=>Array.isArray(x)?x:(keys.map(k=>x?.[k]).find(Array.isArray)||[]);
export const society=(x)=>x?.society||x?.data?.society||x||{};
